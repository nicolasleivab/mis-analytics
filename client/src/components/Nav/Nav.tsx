...
export default function Nav() {
 ...
 const handleKeyPress = useCallback((event, route) => {
    if (event?.key === 'Enter') {
    ...
    <div className={styles.Nav}>
      <Flex justifyContent="space-between" alignItems="center">
        <Flex padding="20px" justifyContent="flex-start" alignItems="center">
          ...
          <Link to={HOME_ROUTE} className={activeLink(HOME_ROUTE)} onKeyDown={(e) => handleKeyPress(e, HOME_ROUTE)}>Home</Link>
          <Link to={DASHBOARD_ROUTE} className={activeLink(DASHBOARD_ROUTE)} onKeyDown={(e) => handleKeyPress(e, DASHBOARD_ROUTE)}>Dashboard</Link>
        </Flex>
        {/* <ThemeToggle /> */}
      </Flex>
    </div>
